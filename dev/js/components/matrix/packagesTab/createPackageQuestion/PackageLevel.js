      ...
      <div>
        <Grid container className="row-item">
          <Grid item xs={2} className="package-level-counts">
            <Typography type="caption">Vas, trend down</Typography>
            <Input
          ...
          <Grid item xs={2} className="package-level-counts">
            <Typography type="caption">VAS, min</Typography>
            <Input
          ...
          <Grid item xs={2} className="package-level-counts">
            <Typography type="caption">Sessions</Typography>
            <Input
          ...
          <Grid item xs={2} className="package-level-counts">
            <Typography type="caption">Package Level Days</Typography>
            <Input
            ...
            >
              {THERAPY.map((item, index) => (
                <MenuItem
                  ...
                  style={{
                    fontWeight:
                      THERAPY.indexOf(item.value) !== -1 ? '500' : '400'
                  }}
                >
                  {item.label}
                </MenuItem>
        ...
        <Grid container className="package-level-exercises">
          <Grid item xs={12}>
            <Typography type="title">Exercises</Typography>
        ...
        {[1, 2, 3, 4].map(order => {
          const exercisesForCurrentPosition = exercises.filter(
            ex => ex.order === order
          );
          return (
            <Grid key={order} container className="package-level-exercises">
              <Grid item xs={12}>
                <Typography type="title">Position {order}</Typography>
                {/*{this.state.loading && <CircularProgress size={20}/>}*/}
              </Grid>
                ...
                {this.state.chooseExercises == order && (
                  <PickPackageExercisesModal
                    ...
                    handleRequestClose={value =>
                      this.openChooseExercises(value)
                    }
                  />
                )}
              </Grid>
