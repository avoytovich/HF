...
class CreateQuestionComponent extends Component {
  ...
  submitQuestion = value => {
    ...
    const validValue = this.createValidateObj(
      { questionKey, questionTitle, question },
      value
    );
    const optional =
      content_type !== "vas"
    ...
    const validateAssets =
      isContentType && !this.validateDiagnosticAssets(files);
  ...
  validateDiagnosticAssets = assets =>
    ...
    (assets && assets["swe"] && assets["swe"].video && true) ||
    false;

  configureQuestionResult = (value, optional) => {
    const { areaIds, answerType, rules, diagnostic_assets, files } = value;
    const filesFinal = {
      en: {
        video_id: null
        //image_id: null
